...
def collect_trajectories(env, policy, steps, partial_observation_idx=1):
    obs = env.reset()
    ...
    for _ in range(steps):
        partial_obs = obs[partial_observation_idx].reshape(-1)  
        obs_tensor = torch.as_tensor(partial_obs, dtype=torch.float32)
        action, log_prob, hidden_state = policy.get_action(obs_tensor, hidden_state)
        next_obs, reward, done, truncated, _ = env.step(action)

        if isinstance(next_obs, tuple):
            next_obs = next_obs[0]

        trajectories.append((partial_obs, action, reward, log_prob.item()))  
...
def compute_gae(trajectories, value_function, gamma, lam):
    ...
    values, _ = value_function(states)  
