...
from Modules import RecurrentPolicyNetwork, RecurrentValueNetwork

# Environment setup
env = gym.make('Pendulum-v1')
act_dim = env.action_space.shape[0]
obs_dim = 1  # Using only angular velocity
...
policy = RecurrentPolicyNetwork(obs_dim, act_dim, hidden_sizes)
value_function = RecurrentValueNetwork(obs_dim, hidden_sizes)
...
for setting_name, (clipping, use_gae) in settings.items():
    ...
    for epoch in range(epochs):
        # Collect trajectories with partial observation
        trajectories = collect_trajectories(env, policy, steps_per_epoch, partial_observation_idx=1)
